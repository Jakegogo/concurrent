...
public class WindowsShortcut
{
    ...
    public WindowsShortcut(File file, String encoding) throws IOException, ParseException {
        InputStream in = new FileInputStream(file);
        try {
            parseLink(getBytes(in), encoding);
        } finally {
            in.close();
        }
    }
     ...
     */
    private void parseLink(byte[] link, String encoding) throws ParseException {
        try {
    ...
    private static String getNullDelimitedString(byte[] bytes, int off, String encoding) {
        int len = 0;
