...
public class NumberSerializer implements Serializer, Opcodes {
    ...
    public void compile(Type type, MethodVisitor mv, AsmContext context) {
        ...
        if (numberClass == null || numberClass == Object.class) {
			...
			mv.visitMethodInsn(INVOKESPECIAL, AsmUtils.toAsmCls(context.getClassName()), "putIntVal", "(Ltransfer/Outputable;Ljava/lang/Number;)V", false);
		} else if (numberClass == long.class || numberClass == Long.class
                || numberClass == AtomicLong.class || numberClass == BigInteger.class) {
            mv.visitVarInsn(ALOAD, 0);
            mv.visitVarInsn(ALOAD, 1);
            mv.visitVarInsn(ALOAD, 4);
            mv.visitMethodInsn(INVOKESPECIAL, AsmUtils.toAsmCls(context.getClassName()), "putLongVal", "(Ltransfer/Outputable;Ljava/lang/Number;)V", false);
        } else {
